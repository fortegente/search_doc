[{if $pages->changePage}]
    ...
    [{if $pages->previousPage }]
        <a class="prev" rel="prev" href="[{$pages->previousPage}]">[{oxmultilang ident="DOCS_PREVIOUS"}]</a>
    [{/if}]
        ...
        [{/foreach}]
    [{if $pages->nextPage }]
        <a class="next" rel="next" href="[{$pages->nextPage}]">[{oxmultilang ident="DOCS_NEXT"}]</a>
    [{/if}]
     </div>
[{/if}]
