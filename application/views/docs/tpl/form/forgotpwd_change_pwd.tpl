 ...
 <form class="js-oxValidate" action="[{ $oViewConf->getSelfActionLink() }]" name="forgotpwd" method="post">
      ...
      <ul class="form clear">
            <li [{if $aErrors.oxuser__oxpassword}]class="oxInValid"[{/if}]>
                <label>[{ oxmultilang ident="NEW_PASSWORD"}]</label>
            ...
            <li class="formSubmit">
                <button id="loginButton" class="forgot_pass_button" type="submit" name="save" value="[{ oxmultilang ident="CHANGE_PASSWORD" }]">[{ oxmultilang ident="CHANGE_PASSWORD" }]</button>
            </li>
      </ul>
    </form>