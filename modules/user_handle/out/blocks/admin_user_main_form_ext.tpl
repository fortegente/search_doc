...
<tr>
    <td class="edittext" width="90">
        [{oxmultilang ident="ZS_USER_PAY_DURATION" }]
    </td>
    <td class="edittext">
        <select name="editval[oxuser__zs_pay_duration]" class="editinput" [{$readonly }]>
        [{foreach from=$pay_duration item=shopitem}]
    <option value="[{$shopitem}]" [{if $shopitem == $edit->oxuser__zs_pay_duration->value}]SELECTED[{/if}]>[{$shopitem}]</option>
        [{/foreach}]
        </select>
    </td>
</tr>
[{if $edit->oxuser__zs_pay_duration->value}]
    <tr>
        <td class="edittext" width="90">
            [{oxmultilang ident="ZS_USER_PAY_DATE" }]
        </td>
        <td class="edittext">
            [{$edit->oxuser__zs_pay_date->value}]
            <input class="edittext" type="checkbox" name="editval[update_pay_date]">

        </td>
    </tr>
    <input type="hidden" value="[{$edit->oxuser__zs_pay_date->value}]">
[{/if}]
<tr>
    <td class="edittext">
